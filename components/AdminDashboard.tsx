...
const AdminDashboard: React.FC = () => {
  ...
  const handleAction = async (orderId: string, action: 'verify' | 'hold' | 'cancel') => {
    ...
    if (!confirm(`Proceed with ${labels[action]}? Notification will be sent to the registered email.`)) return;
    
    setProcessingId(orderId);
    try {
      await api.processOrderAction(orderId, action);
      alert(`${labels[action]} successful. Email dispatched.`);
      await fetchData();
  ...
  const filteredOrders = useMemo(() => {
    return data.orders.filter(o => {
      ...
      const exec = "Admin System"; // Default executive mapping
  ...
  const salesRegister = useMemo(() => {
    ...
    filteredOrders.filter(o => o.status === 'Completed').forEach(order => {
      ...
      order.voucherCodes.forEach(code => {
        // Find the original upload date from the vault
  ...
  const stockReport = useMemo(() => {
    ...
    return categories.map(cat => {
      ...
      // Simulated opening stock for reporting purposes
    ...
    <div className="max-w-7xl mx-auto px-6 view-container pb-24 bg-white">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 mb-16">
        <div>
          ...
          <p className="text-slate-500 mt-4 font-bold uppercase text-[10px] tracking-widest">Global Fulfillment Authority • {currentUser?.email}</p>
        </div>
        <div className="flex bg-slate-50 p-2 rounded-[2rem] border border-slate-200 shadow-inner">
          {availableTabs.map(tab => (
            ...
            >
              {tab === 'verification' ? 'Teller Desk' : tab === 'reports' ? 'Reporting Hub' : tab.charAt(0).toUpperCase() + tab.slice(1)}
            </button>
          ))}
        </div>
      </div>
      ...
      {activeTab === 'verification' && (
        <div className="bg-white p-12 rounded-[4rem] border border-slate-200 shadow-2xl overflow-hidden">
          <h2 className="text-2xl font-black mb-10 text-slate-900 uppercase tracking-tighter">Order Processing Stream</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50 text-[10px] font-black uppercase text-slate-500 tracking-[0.2em] border-y border-slate-100">
                  ...
                  <th className="px-8 py-6 text-right">Fulfillment Options (Auto-Mail)</th>
                </tr>
              </thead>
                ...
                {data.orders.filter(o => o.status === 'Pending').map(o => (
                  <tr key={o.id} className="hover:bg-slate-50 transition-all">
                    ...
                    <td className="px-8 py-6">
                      ...
                      <div className="text-[10px] text-slate-500 font-mono italic">{o.customerEmail}</div>
                    </td>
            ...
            {data.orders.filter(o => o.status === 'Pending').length === 0 && <div className="p-20 text-center text-slate-400 italic font-bold">No active fulfillment requests.</div>}
          </div>
        </div>
      )}
        ...
        <div className="space-y-12">
          <div className="bg-slate-900 p-8 rounded-[3.5rem] shadow-2xl text-white">
            <div className="flex flex-wrap gap-4 mb-10">
              {(['sales', 'stock', 'buyer', 'executive', 'audit', 'search'] as ReportType[]).map(r => (
                <button 
                  ...
                  className={`px-8 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${reportType === r ? 'bg-unicou-orange border-unicou-orange' : 'bg-transparent border-white/20 hover:border-white'}`}
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-6">
              ...
              <div>
                <label className="block text-[9px] font-black uppercase tracking-widest mb-2 opacity-50">Executive</label>
              ...
              <div className="flex items-end">
                <button onClick={() => window.print()} className="w-full py-3 bg-white text-slate-950 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl hover:bg-emerald-50 transition-all">Download PDF</button>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-12 rounded-[4rem] border border-slate-200 shadow-2xl overflow-hidden min-h-[600px]">
            {reportType === 'sales' && (
              <div className="overflow-x-auto">
                <h3 className="text-xl font-black mb-8 uppercase tracking-tighter">Mission Sales Register</h3>
                ...
                <table className="w-full text-left text-[11px]">
                  <thead>
                    <tr className="bg-slate-50 font-black uppercase text-slate-500 tracking-widest border-y border-slate-100">
                      ...
                      <th className="px-10 py-6">Purchased (Replenished)</th>
                      <th className="px-10 py-6">Sold (Dispatched)</th>
                      <th className="px-10 py-6">Closing Stock (Live)</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50">
              ...
              <div className="max-w-3xl mx-auto py-12">
                 ...
                 <div className="relative mb-12">
                   <input 
                    ...
                    placeholder="INPUT RAW VOUCHER STRING..."
                    className="w-full bg-slate-50 border border-slate-200 rounded-[2rem] p-8 text-xl font-mono font-black text-unicou-navy outline-none focus:border-unicou-orange shadow-inner"
                   />
                 </div>
                 <div className="space-y-4">
                   {data.codes.filter(c => c.code.includes(searchCode) && searchCode.length > 2).map(c => (
                     <div key={c.id} className="p-8 bg-slate-50 border border-slate-200 rounded-[2.5rem] flex justify-between items-center group hover:border-unicou-navy transition-all">
                       <div>
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Code Mapping</p>
                       ...
                       <div className="text-right">
                          ...
                          <p className="text-[9px] text-slate-400 mt-2">ID: {c.productId}</p>
                       </div>
                     </div>
                   ))}
                   {searchCode.length > 2 && data.codes.filter(c => c.code.includes(searchCode)).length === 0 && (
                     <div className="p-20 text-center text-slate-300 italic uppercase font-bold">Registry miss. Code node not found.</div>
                   )}
                 </div>
              </div>
            )}
            ...
            {reportType === 'audit' && (
              <div className="space-y-8">
                <h3 className="text-xl font-black uppercase tracking-tighter">Fulfillment Audit Trail</h3>
                {data.orders.slice(0, 15).map(o => (
                  <div key={o.id} className="p-8 bg-slate-50 border border-slate-100 rounded-[2.5rem] relative overflow-hidden group hover:border-unicou-navy transition-all">
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="bg-white p-5 rounded-2xl border border-slate-200">
                           <p className="text-[8px] font-black text-slate-400 uppercase mb-1">Phase 1: Entry</p>
                           <p className="text-[10px] font-bold">Lead Initialization</p>
                        </div>
                        <div className="bg-white p-5 rounded-2xl border border-slate-200">
                           <p className="text-[8px] font-black text-slate-400 uppercase mb-1">Phase 2: Intent</p>
                           <p className="text-[10px] font-bold">Checkout: {o.customerEmail}</p>
                        </div>
                        <div className="bg-white p-5 rounded-2xl border border-slate-200">
                           <p className="text-[8px] font-black text-slate-400 uppercase mb-1">Phase 3: Settlement</p>
                           <p className="text-[10px] font-bold text-emerald-600">{o.paymentMethod} Verified</p>
                        </div>
                        <div className="bg-white p-5 rounded-2xl border border-slate-200">
                           <p className="text-[8px] font-black text-slate-400 uppercase mb-1">Phase 4: Release</p>
                           <p className={`text-[10px] font-bold ${o.status === 'Completed' ? 'text-unicou-orange' : 'text-slate-400'}`}>{o.status === 'Completed' ? 'Vouchers Dispatched' : 'Awaiting Release'}</p>
                        </div>
                     </div>
                  </div>
                ))}
      ...
      {activeTab === 'staff' && isAdmin && (
        <div className="bg-white rounded-[4rem] border border-slate-200 overflow-hidden shadow-2xl">
           <div className="p-12 border-b border-slate-100 bg-slate-50/50">
             <h2 className="text-3xl font-display font-black text-slate-900 tracking-tighter uppercase">Authority Registry</h2>
           </div>
           <table className="w-full text-left">

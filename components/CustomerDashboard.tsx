    ...
    <div className="max-w-7xl mx-auto px-6 py-12 bg-white">
      <div className="text-center mb-20 animate-in slide-in-from-top-4 duration-700">
        ...
        <p className="text-xl text-slate-600 max-w-2xl mx-auto leading-relaxed font-bold italic">
          Hi {user.name}! Your UniCou student portal is ready. Learn, prepare, and move closer to your international career goals!
        </p>
        ...
        <div className="flex justify-center mt-12">
          <div className="flex bg-slate-50 p-2 rounded-[2.5rem] border border-slate-200 shadow-inner overflow-x-auto no-scrollbar">
            <button 
              ...
              className={`px-10 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === 'vouchers' ? 'bg-white text-unicou-navy shadow-xl border border-slate-200' : 'text-slate-500 hover:text-unicou-navy'}`}
            >Voucher Vault</button>
            <button 
              onClick={() => setActiveTab('academy')}
              className={`px-10 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === 'academy' ? 'bg-white text-unicou-navy shadow-xl border border-slate-200' : 'text-slate-500 hover:text-unicou-navy'}`}
            >Learning Hub</button>
            <button 
              onClick={() => setActiveTab('results')}
              className={`px-10 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === 'results' ? 'bg-white text-unicou-navy shadow-xl border border-slate-200' : 'text-slate-500 hover:text-unicou-navy'}`}
            >Exam Results</button>
          </div>
        </div>
      </div>
      ...
      <div className="animate-in fade-in duration-700 pb-20">
          ...
          <div className="grid grid-cols-1 gap-12">
            {orders.length === 0 ? (
              <div className="bg-slate-50 p-32 rounded-[4rem] border-4 border-dashed border-slate-200 text-center">
                <p className="text-slate-400 text-lg font-bold italic mb-10">Your voucher list is empty.</p>
                <button onClick={() => onNavigate({ type: 'store' })} className="px-10 py-5 bg-unicou-navy text-white rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-2xl hover:bg-slate-950 transition-all">VISIT VOUCHER STORE</button>
              </div>
            ) : (
              orders.map(o => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {courses.length === 0 ? (
              <div className="col-span-full bg-slate-50 p-32 rounded-[4rem] border-4 border-dashed border-slate-200 text-center">
                <p className="text-slate-400 text-lg font-bold italic mb-10">No enrolled courses found.</p>
                <button onClick={() => onNavigate({ type: 'lms-dashboard' })} className="px-10 py-5 bg-unicou-navy text-white rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-2xl hover:bg-slate-950 transition-all">EXPLORE COURSES</button>
              </div>
            ) : (
              courses.map((course, idx) => (
                <div 
                  key={course.id} 
                  className="bg-white rounded-[3rem] overflow-hidden border border-slate-200 flex flex-col hover:border-unicou-orange/20 transition-all group shadow-xl animate-in fade-in slide-in-from-bottom-4 duration-500"
        ...
        {activeTab === 'results' && (
          <div className="space-y-12">
            {results.length === 0 ? (
              <div className="bg-slate-50 p-32 rounded-[4rem] border-4 border-dashed border-slate-200 text-center">
                <p className="text-slate-400 text-lg font-bold italic">Awaiting synchronized exam results from your mock tests.</p>
              </div>
            ) : (
              results.map(r => (
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-start gap-12 mb-16 relative z-10">
                    <div className="max-w-2xl">
                      <div className="flex items-center gap-4 mb-6">
                        ...
                        <span className="text-slate-400 text-[10px] font-mono font-black tracking-widest uppercase">ID: {r.id}</span>
                      </div>
                      <h3 className="text-4xl font-display font-black text-slate-950 tracking-tighter uppercase">{r.testTitle}</h3>
                      <p className="text-slate-600 text-base mt-4 font-bold italic leading-relaxed">Completed on {new Date(r.timestamp).toLocaleString()}. Verified by UniCou Trainers.</p>
                    </div>
                    {r.overallBand && (
                      <div className="text-center md:text-right bg-slate-50 p-8 rounded-[3rem] border border-slate-100 shadow-inner">
                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-3">OVERALL SCORE</p>
                    ...
                    {r.skillScores.map(ss => (
                      <div key={ss.skill} className="bg-slate-50 border border-slate-100 p-10 rounded-[3rem] hover:border-unicou-navy transition-all shadow-sm">
                        ...
                        <p className="text-3xl font-black text-slate-950 font-mono tracking-tighter">
                          {ss.isGraded ? (ss.band || `${ss.score}/${ss.total}`) : 'PENDING'}
                        </p>
                        {ss.isGraded && ss.feedback && (
                          <div className="mt-8 pt-8 border-t border-slate-200">
                             <p className="text-[9px] font-black text-unicou-orange uppercase tracking-widest mb-3">Feedback</p>

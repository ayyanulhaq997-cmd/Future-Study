    ...
    <div className="max-w-[440px] mx-auto animate-slide-up bg-white py-16 px-4">
      <div className="bg-white p-12 md:p-14 rounded-[4rem] border border-slate-100 shadow-3xl relative overflow-hidden">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-slate-400 text-[10px] mt-6 font-black uppercase tracking-[0.4em]">Establishing Protocol Node</p>
        </div>

        <form onSubmit={handleLoginSubmit} className="space-y-8">
          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 ml-1">User ID</label>
            <input 
              type="text" required value={userId}
              onChange={(e) => setUserId(e.target.value)}
              placeholder="e.g. user@unicou.uk"
              className="w-full bg-slate-50 border border-slate-100 rounded-3xl p-5 text-slate-900 outline-none focus:border-unicou-navy focus:bg-white transition-all shadow-inner font-bold text-lg"
            />
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <input 
                ...
                placeholder="••••••••"
                className="w-full bg-slate-50 border border-slate-100 rounded-3xl p-5 pr-14 text-slate-900 outline-none focus:border-unicou-navy focus:bg-white transition-all shadow-inner font-bold text-lg"
              />
              <button 
                type="button" 
                onClick={() => setShowPassword(!showPassword)}
                className="absolute right-5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-unicou-navy transition-colors p-2"
              >
                {showPassword ? (
        ...
        <div className="text-center pt-10 border-t border-slate-50 mt-12">
          <p className="text-xs text-slate-500 font-bold uppercase tracking-tight">New Student? <button type="button" onClick={onNavigateToSignup} className="text-unicou-orange font-black hover:underline underline-offset-4 transition-all ml-1">Sing Up</button></p>
        </div>
      </div>
    </div>
  );

    ...
    <div className="max-w-[1600px] mx-auto px-6 pb-24 bg-white">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 mb-16 pt-12">
        <div>
          ...
          <p className="text-slate-500 mt-4 font-bold uppercase text-xs tracking-widest">Operator Node: {user.name} • <span className="text-unicou-navy font-black">Verification Authority</span></p>
        </div>
        <div className="flex bg-slate-50 p-2 rounded-[2rem] border border-slate-200 shadow-inner">
          ...
          <button onClick={() => setActiveTab('orders')} className={`px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'orders' ? 'bg-white text-unicou-navy shadow-lg' : 'text-slate-400'}`}>Verification Feed</button>
        </div>
      </div>
      ...
      <div className="bg-white p-10 rounded-[4rem] border border-slate-200 shadow-2xl overflow-hidden min-h-[600px]">
        {activeTab === 'orders' && (
          <div className="overflow-x-auto">
             <h3 className="text-xl font-black mb-8 uppercase tracking-tighter">ii. Sales Agent Dash Board (Live Fulfillment Node)</h3>
             <table className="w-full text-left text-[10px]">
              <thead>
                <tr className="bg-slate-50 font-black uppercase text-slate-500 tracking-widest border-y border-slate-100">
                  ...
                  <th className="px-4 py-6 text-center">Verify & Deliver Action</th>
                  <th className="px-4 py-6 text-center">Dispatch Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 uppercase">
                {orders.map(o => (
                  <tr key={o.id} className="hover:bg-slate-50 font-bold text-slate-600 transition-colors">
                    ...
                    <td className="px-4 py-6">
                       <div className="flex items-center justify-center gap-1.5">
                          {o.status === 'Pending' ? (
                            <>
                              <button onClick={() => handleAction(o.id, 'Approved')} className="px-3 py-1.5 bg-emerald-500 text-white rounded-lg text-[8px] font-black uppercase hover:bg-emerald-600 shadow-sm" title="Verify and Deliver">Verify</button>
                              <button onClick={() => handleAction(o.id, 'Hold')} className="px-3 py-1.5 bg-amber-500 text-white rounded-lg text-[8px] font-black uppercase hover:bg-amber-600 shadow-sm" title="Further Verification">Further Verify</button>
                              <button onClick={() => handleAction(o.id, 'Rejected')} className="px-3 py-1.5 bg-red-600 text-white rounded-lg text-[8px] font-black uppercase hover:bg-red-700 shadow-sm" title="Reject">Reject</button>
                            </>
                          ) : (
                            <div className="flex items-center gap-2">
                               <div className={`w-2 h-2 rounded-full ${o.status === 'Approved' ? 'bg-emerald-500' : 'bg-red-500'}`} />
                               <span className="text-[9px] font-black">{o.status}</span>
                            </div>
                          )}
                       </div>
                    </td>
                    <td className="px-4 py-6 text-center">
                       <span className="text-[8px] font-mono text-slate-400">{o.deliveryTime || '---'}</span>
                    </td>
                  </tr>
                ))}
              </tbody>

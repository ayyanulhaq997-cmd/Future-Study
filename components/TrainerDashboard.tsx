  ...
  return (
    <div className="max-w-7xl mx-auto px-6 view-container pb-24 bg-white">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 mb-16">
        <div>
          <h1 className="text-5xl font-display font-black tracking-tight text-slate-900 leading-none uppercase">Academic Trainer <span className="text-unicou-orange">Portal</span></h1>
          <p className="text-slate-500 mt-4 font-bold uppercase text-xs tracking-widest">
            Fulfillment Authority: <span className="text-unicou-navy font-black">{user.email}</span> • Registry: <span className="text-unicou-orange">Academic Evaluation Node</span>
          </p>
        </div>
        <div className="flex items-center gap-4">
          <div className="bg-slate-50 p-6 rounded-[2.5rem] border border-slate-200 shadow-inner">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] mb-1">Assessment Load</p>
            <div className="flex items-center gap-3">
              <span className="text-3xl font-display font-black text-unicou-navy leading-none">{pendingSubmissions.length}</span>
              <span className="text-[9px] font-bold text-emerald-600 uppercase">Live Submissions</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-4">
          <div className="bg-white rounded-[3.5rem] border border-slate-200 overflow-hidden shadow-2xl flex flex-col h-[700px]">
            <div className="p-8 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
               <h3 className="text-sm font-black uppercase tracking-widest text-slate-900">Evaluation Queue</h3>
               <div className="flex items-center gap-2">
                 <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" />
                 <span className="text-[9px] font-black text-slate-400 uppercase">Real-time Node</span>
               </div>
            </div>
            ...
            <div className="overflow-y-auto flex-grow no-scrollbar divide-y divide-slate-50">
              {pendingSubmissions.length === 0 ? (
                <div className="p-20 text-center flex flex-col items-center justify-center h-full">
                  ...
                  <p className="text-slate-400 italic font-bold uppercase tracking-widest text-[10px]">No pending assessments found.</p>
                </div>
              ) : (
        ...
        <div className="lg:col-span-8">
          {selectedItem ? (
            <div className="bg-white rounded-[4rem] border border-slate-200 p-12 md:p-16 shadow-2xl relative overflow-hidden animate-in zoom-in-95 duration-500">
               ...
               <div className="relative z-10 mb-12 pb-12 border-b border-slate-100 flex justify-between items-start">
                  <div>
                    <div className="flex items-center gap-3 mb-4">
                       <span className="px-4 py-1.5 bg-unicou-navy text-white text-[9px] font-black uppercase tracking-widest rounded-full shadow-lg">Manual Grading Active</span>
                    ...
                    <p className="text-slate-500 text-base mt-4 font-bold italic leading-relaxed">
                      Evaluating <span className="text-unicou-navy">{selectedItem.userName}</span> for <span className="text-unicou-navy">{selectedItem.testTitle}</span>. 
                      Adhere strictly to standard academic rubrics and quality norms.
                    </p>
                  </div>
               ...
               <div className="space-y-12 relative z-10">
                  <div className="space-y-4">
                    <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Question Context</h4>
                    <div className="bg-unicou-navy/5 border border-unicou-navy/10 rounded-3xl p-8 leading-relaxed text-unicou-navy font-bold italic text-base">
                      {selectedItem.questionText || "Academic prompt missing context node."}
                    </div>
                  </div>

                  <div className="space-y-4">
                     <div className="flex justify-between items-center px-2">
                        <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Student Response Node</h4>
                        <span className="text-[10px] font-black text-slate-900 uppercase font-mono bg-slate-50 px-3 py-1 rounded-full border border-slate-100">{selectedItem.studentAnswer.split(/\s+/).length} Words</span>
                     </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-12 pt-12 border-t border-slate-100">
                     <div className="md:col-span-5 space-y-8">
                        <div>
                           <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-4 ml-2">Final Evaluation Score (Max: {selectedItem.maxScore})</label>
                           <input 
                        ...
                        <div className="p-8 bg-unicou-navy rounded-[2.5rem] shadow-xl text-white">
                           <p className="text-[10px] font-black text-unicou-vibrant uppercase tracking-widest mb-4">Quality Rubric</p>
                           <ul className="space-y-3 text-[10px] font-black uppercase tracking-widest opacity-70 italic">
                             <li>○ Grammatical Complexity</li>
                             <li>○ Lexical Precision Node</li>
                             <li>○ Logical Coherence Flow</li>
                             <li>○ Task Fulfillment Depth</li>
                           </ul>
                        </div>
                        ...
                        >
                           ...
                           {isRecording ? 'Stop Audio Feed' : 'Attach Biometric Feedback'}
                        </button>
                     </div>

                     <div className="md:col-span-7 flex flex-col h-full">
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-4 ml-2">Academic Counselor Feedback</label>
                        <textarea 
                          ...
                          placeholder="Provide deep academic feedback for the student..."
                          value={feedback}
            ...
            <div className="h-[700px] bg-slate-50 rounded-[5rem] border-4 border-dashed border-slate-100 flex flex-col items-center justify-center p-32 text-center group">
               ...
               <h3 className="text-2xl font-black text-slate-950 uppercase tracking-tighter mb-4">Academic Terminal Ready</h3>
